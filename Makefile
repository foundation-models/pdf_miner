...
# Use bash as the shell for Makefile
SHELL := /bin/bash
# Load .env if present
ifneq (,$(wildcard .env))
	include .env
...
# Task to create logs directory, set permissions, and start n8n
start_n8n:
	@echo "Setting up logs directory and starting n8n..."
	@mkdir -p $(N8N_LOGS_DIR)
	@chmod 755 $(N8N_LOGS_DIR)
	@echo "Loading NVM and starting n8n..."
	bash -c 'source "$(NVM_DIR)/nvm.sh" && \
	  nvm use 18 || nvm install 18 && \
	  export N8N_RUNNERS_ENABLED=true && \
	  export NODE_FUNCTION_ALLOW_EXTERNAL="pdfjs-dist,pdf-parse,node-fetch" && \
	  npx n8n@$(N8N_VERSION)'
.PHONY: start_n8n
